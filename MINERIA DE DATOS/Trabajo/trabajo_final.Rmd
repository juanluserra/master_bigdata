...
data.train <- data.train %>% select(-c(contamination_2levels, PM2.5_annualMean, kreis, year))
data.test <- data.test %>% select(-c(contamination_2levels, PM2.5_annualMean, kreis, year))
x <- scale(as.matrix(data.train %>% select(-contamination)))
y <- as.integer(as.factor(data.train$contamination)) - 1L
n <- ncol(x)
K <- 4
...
model <- keras_model_sequential()
model %>%
    layer_dense(units = best$units, activation = "relu", input_shape = n) %>%
...
# Predecimos el conjunto de test
freqs <- as.list(table(y) / sum(table(y)))
probs <- model %>% predict(x.test, verbose = 0)
pred_cls <- apply(probs, 1, which.max) - 1L
pred_cls <- rep(0, nrow(probs))
for (i in 1:nrow(probs)) {
    if (probs[i, 4] >= freqs[4]) {
        pred_cls[i] <- 3
    } else if (probs[i, 3] >= freqs[3]) {
        pred_cls[i] <- 2
    } else if (probs[i, 2] >= freqs[2]) {
        pred_cls[i] <- 1
    } else {
        pred_cls[i] <- 0
    }
