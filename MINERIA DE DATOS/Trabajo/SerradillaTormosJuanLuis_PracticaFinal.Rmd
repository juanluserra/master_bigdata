...
# Insalamos los paquetes que no estén instalados
...
En esta práctica se ha analizado un dataset de contaminación del aire en regiones de Alemania. 

Para el preprocesamiento de datos se han eliminado los valores `NA`, duplicados e inválidos, aunque para estos últimos se ha investigando con `aggr` si podían eliminarse. También se ha eliminado directamente la variable `kreis_code` por no aportar información, y la variable `NO2_hrOver200` por tener una varianza muy baja. Este último análisis se ha hecho gracias a nearZeroVar de `caret`. Finalmente, en el análisis descriptivo se ha analizado la distribución de las categorías contaminación baja y alta en todas las variables (numéricas y categóricas), sacando así conclusiones de las características de las zonas con alta y baja contaminación de P2.5. Finalmente, de este análisis descriptivo se ha visto que la variable más relacionada con la contaminación PM10 (con un exceso de esta en las zonas contaminadas), además de que los datos de contaminación se juntan en su gran mayoría en un solo año (2003), y el resto se reparto solo en dos años más (2007 y 2011). 

En el entrenamiento de modelos predictivos primero se han seleccionado las variables válidas del dataset y se han balanceado los datos. Se han descartado las variables `kreis`, `year` y `scenario` por no tener capacidad predictiva, de forma que nos quedamos con 9 predictores. Al no tener una alta cantidad de predictores, no se ha hecho una selección de variables, ya que perderíamos información innecesariamente. Luego, para el balanceo de datos se ha hecho un upsampling con SMOTE y un downsampling con selección aleatoria. El upsampling de SMOTE se ha hecho con `UBL` y no con `smotefamily`, ya que permite hacer un upsampling de varias clases a la vez, que era necesario en nuestro caso. Este upsampling  y downsampling se ha hecho solo con el conjunto de train, ya que el conjunto te test debe permanecer intacto para representar fielmente la realidad.

Una vez se tienen los datos balanceados se pasa a la creación de modelos. Para seleccionar los hiperparámetros se ha usado el método de la validación cruzada con una cantidad de 10 folds. En los modelos que usan el paquete caret se escoge directamente el modelo con el mejor kappa y accuracy; mientras, en el caso de la red neuronal se ha hecho un bucle de validación cruzada de 10 folds y, cuando se tienen los resultados, se usa el F1 Score para seleccionar el mejor modelo. Sin embargo, no se coge directamente el modelo con mejor F1 Score, se usa el método del error estándar, seleccionando así el modelo más simple pero con mejor desempeño dentro de los modelos con mejor F1 Score. Luego, para la comparar modelos se ha sacado el índice Kappa y también el F1 Score medio de las clases, además de las matrices de confusión. Tras analizar las matrices de confusión y las métricas se verá como los mejores modelos para el problema con Random Forest y Boosting, permitiendo además una buena interpretabilidad con la importancia de variables. Al hacer un análisis de importancia con estos modelos se ve, además, como al igual se analiza anteriormente las variables de PM10 son las más importantes con diferencias, seguidas de NO2; además, las variables de medias anuales son más relevantes que las variables de conteo de días u horas.
...
Se puede observar como los valores negativos representan un 2.38% del dataframe. Esto es bastante más que el 0.19% que suponían los valores NA, por lo que en vez de eliminarlos vamos a usar el paquete `VIM` para ver si podemos imputar estos valores. Vamos a convertir estos valores a `NA` y luego observar su distribución con `aggr`.
```{r}
...
Podemos ver en los resultados que hay dos variables con un `freqRatio` muy alto: `NO2_hrOver200` y `O3_daysOver120`, aunque la primera es mucho mayor a la segunda. De primeras esto es un mal indicativo, sin embargo, las dos variables tienen un `percentUnique` alto, por lo que el algoritmo no las clasifica con `TRUE` en `nzv`. La segunda variable tiene un `freqRatio` de 11, que junto a su alto `percentUnique` de 62 indica que es una variable válida. Por otro lado, la primera variable tiene un `freqRatio` enorme en comparación con el resto (de 250) y, aunque tenga un `percentUnique` del 48.6 (lo cual no es bajo), sí que es mucho menor que el siguiente valor más pequeño (57.4), de casi 10 puntos menos. Por lo tanto, vamos a eliminar del dataset la variable `NO2_hrOver200`.
```{r}
...
Para poder responder, lo primero de todo es crear una variable binaria que nos indique si la zona tiene alta o baja contaminación por PM2.5. Como se indica en el documento de la práctica, la contaminación puede dividirse en Baja, Moderada, Insalubre y Muy Insalubre. Por lo tanto, vamos a realizar una variable con estas categorías y, después, agruparemos Baja e Insalubre en una sola categoría, y Moderada y Muy Insalubre en otra. Para ello, vamos a usar la función `cut` de R.
```{r}
...
Ahora que tenemos la variable categórica, vamos a ver las distribuciones de las variables numéricas en función del nivel de contaminación. También vamos a ver la distribución de las variables categóricas (año y scenario) en función del nivel de contaminación, buscando alguna relación.
```{r}
...
En el gráfico de las distribuciones de las variables numéricas podemos ver 3 casos: no hay diferencia entre distribuciones, hay diferencia en anchura, hay diferencia en posición y anchura. Vamos a analizar cada uno de estos casos.
...
- **Distribuciones solapadas:** En este caso tenemos a `NO2_annualMean`, `NO_annualMean`, `O3_annualMean`, `O3_daysOver120`, `O3_dailyMaxAnnualMean`. En estos casos la media es igual o similar tanto con o sin contaminación, pero la anchura de la distribución varía, por loq que hay regiones de valores que corresponden en su mayoría a una de las dos categorías. En el caso de `NO2_annualMean`, `NO_annualMean` y `O3_annualMean`, valores muy extremos de concentraciones sde partículas tienden a indicar alta contaminación de PM2.5. Por otro lado, en el caso de las variables relacionadas con la cantidad de días u horas (`O3_daysOver120`, `O3_dailyMaxAnnualMean`), una alta cantidad de días u horas con concentraciones altas de O3 tienden a indicar alta contaminación de PM2.5.
...
Ahora sí, vamos a crear los modelos. Para ello se usará el paquete `caret` (a excepción de la red neuronal que usará `keras`). Estos modelos se entrarán con validación cruzada de 10 folds, escogiendo así los mejores parámetros. Los modelos que se van a usar son:
...
Se usará el paquete `doParallel` para paralelizar el entrenamiento de los modelos, minimizando el tiempo de cómputo. Los modelos que se usarán en el trabajo a entregar no se calcularán en el momento, sino que se habrán calculado previamente y simplemente habrá que cargar los archivos que los contienen.
...
# Guardamos el modelo
saveRDS(rpart.cv, file = "rpart.rds", compress = TRUE)
```
...
# Guardamos el modelo
saveRDS(rf.cv, file = "rf.rds", compress = TRUE)
```
...
# Guardamos el modelo
saveRDS(gbm.cv, file = "gbm.rds", compress = TRUE)
```
...
# Guardamos el modelo
saveRDS(svm.cv, file = "svm.rds", compress = TRUE)
```
...
folds <- createFolds(y.train, k = 10, returnTrain = FALSE)
...
results <- foreach(
    ...
    for (j in seq_along(folds)) {
        ...
        f1s[j] <- mean(2 * cm$byClass[, "Pos Pred Value"] * cm$byClass[, "Sensitivity"] / (cm$byClass[, "Pos Pred Value"] + cm$byClass[, "Sensitivity"]))
    }
...
saveRDS(results, file = "nnet.rds", compress = TRUE)
```
...
results <- results[!is.na(results$mean_f1), ]
...
best <- results[results$f1_greater > max(results$f1_lower, na.rm = TRUE), ]
...
De los modelos más simples (de 1 capa) vamos a elegir el de 128 unidades y learning rate 0.01 con 30 epochs, ya que dentro de los modelos simples tener 128 unidades no supone mucho más costo computacional que tener 64 o 32, y el resultado de 50 epochs no está muy por debajo que el de 100 epochs.
```{r}
...
rpart.f1 <- 2 * rpart.conf$byClass[, "Sensitivity"] * rpart.conf$byClass[, "Pos Pred Value"] / (rpart.conf$byClass[, "Sensitivity"] + rpart.conf$byClass[, "Pos Pred Value"])
rf.f1 <- 2 * rf.conf$byClass[, "Sensitivity"] * rf.conf$byClass[, "Pos Pred Value"] / (rf.conf$byClass[, "Sensitivity"] + rf.conf$byClass[, "Pos Pred Value"])
gbm.f1 <- 2 * gbm.conf$byClass[, "Sensitivity"] * gbm.conf$byClass[, "Pos Pred Value"] / (gbm.conf$byClass[, "Sensitivity"] + gbm.conf$byClass[, "Pos Pred Value"])
svm.f1 <- 2 * svm.conf$byClass[, "Sensitivity"] * svm.conf$byClass[, "Pos Pred Value"] / (svm.conf$byClass[, "Sensitivity"] + svm.conf$byClass[, "Pos Pred Value"])
nnet.f1 <- 2 * nnet.conf$byClass[, "Sensitivity"] * nnet.conf$byClass[, "Pos Pred Value"] / (nnet.conf$byClass[, "Sensitivity"] + nnet.conf$byClass[, "Pos Pred Value"])
