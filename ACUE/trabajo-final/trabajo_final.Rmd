...
Ahora que hemos definido los grupos y los incentivos, hay que simular los resultados de la campaña. Para ello, vamos a estimar porcentajes modestos de éxito en la campaña. Supongamos:
- **Grupo 1 de no desertores**: En torno al 10% se pasan al siguinete grupo (el grupo 2) de no desertores. Suponiendo que el 100% de los desertores del grupo 1 son de este grupo de no desertores, este grupo de desertores se puede ver reducido en un 10% (es decir, el 90% de los desertores del grupo 1 pertenecerán al grupo 2 de no desertores).
- **Grupo 2 de no desertores**: Al ser un grupo con más confianza en la empresa, será más fácil hacer que no deserten. Las políticas de permanencia hacen que el 10% se quede en el grupo 2 y que el 5% pase al grupo 3 de no desertores. Suponiendo que el grupo 2 de desrtores se compone por 2/3 del grupo 2 de no desertores y 1/3 del grupo 3 de no desertores, esto hace que el grupo 2 de desertores se vea reducido un 
...
$$
    ...
    P(t_0; B_1 \rightarrow B_2) = 0.05 \\
    P(t_0; B_2 \rightarrow A_2) = 0.05 \\
    P(t_0; B_2 \rightarrow B_3) = 0.10 \\
    P(t_0; B_3 \rightarrow A_2) = 0.01
$$
...
probs$B1toA1 <- c(0.10, 0.10)
probs$B1toB2 <- c(0.00, 0.05)
probs$B2toA2 <- c(0.10, 0.05)
probs$B2toB3 <- c(0.00, 0.10)
probs$B3toA2 <- c(0.05, 0.01)
```

Ahora que tenemos los datos, vamos a simular el flujo de clientes. Supondremos un aumento inicial de los clientes de un 10%.
```{r}
# Flujos con campaña
clientes$B1[2] <- clientes$B1[1] + (clientes$B1[1] * 0.10) * (1 - probs$B1toA1[2] - probs$B1toB2[2])
...
# Flujos sin campaña
clientes$B1[3] <- clientes$B1[1] + (clientes$B1[1] * 0.10) * (1 - probs$B1toA1[1] - probs$B1toB2[1])
