...
def usage():
    ...
    f("Usage: {os.path.basename(__file__)} --host <host> --port <port> --mqttHost " +
...
if os.path.isfile(pidfile):
    ...
    opid = int(oldpid)
    f("PID file {pidfile} already exists, killing the process {oldpid}")
...
def record():
    ...
    try:
        ...
        print(f"Notificación recibida en {server_url}:")
        print(json.dumps(data, indent=2, ensure_ascii=False))
        print("-" * 40)
    except Exception:
        print("  -> No se pudo parsear JSON de la petición") 
